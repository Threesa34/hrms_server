...
module.exports = {
    ...
    resetAgentsPassword: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            connection.acquire(function(err, con){
                ...
                con.query('SELECT email, name FROM `advertising_partners` WHERE id = ?', [req.body[0]], function(err, agentEmail){
                    if(err)
                    {
                        res.send({
                            status:0,
                ...
                con.query('UPDATE `advertising_partners` SET password = ? WHERE id = ?', [password, req.body[0]], function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({
    ...
    getRedeemTransactinsList: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            connection.acquire(function(err, conn){
            conn.query('SELECT `id`, `redeem_points`, `createddate` FROM `partner_creditpointsredeem` WHERE `agentid` = (SELECT employees.agentid FROM employees WHERE employees.id = ? LIMIT 1);', [req.decoded.logedinuser.id], function(err, result){
                if(err)
                {
                    res.send({
                        status:0,
