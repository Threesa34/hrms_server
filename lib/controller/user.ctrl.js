...
module.exports = {
    ...
    {
        connection.acquire(function(err, con){
            con.query('SELECT `id`,`name`, allow_bg_location,(SELECT role_master.name from role_master WHERE role_master.id = employees.role) AS `rolename`, `role`, (SELECT designation_master.name from designation_master WHERE designation_master.id = employees.designation) AS `designationname`, `designation`, `companyid`,`firstlogin` FROM `employees` WHERE `uuid` = ?',[req.body.uuid], function(err,result){
                if(err)
                {
                    console.log(err)
                    res.send(
