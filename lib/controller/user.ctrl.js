...
module.exports = {
    ...
    getOfficeLocations: function(req, res)
    {
        res.send([
            ...
            {lat: 19.195324, lang: 72.955278, location: 'centrum'}
            ])
    },
    ...
    getUsersList: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            connection.acquire(function(err, con){
                ...
                {
                    var sql = 'SELECT `id`,`name`,`gender`, CONCAT(`mobile1`,CASE WHEN  mobile2 > 0 THEN CONCAT(" / ",mobile2) ELSE "" END) AS mobiles, `email`,DATE_FORMAT(`dob`,"%D %M, %Y") AS  birth_date,DATE_FORMAT(`doj`,"%D %M, %Y") AS  joining_date, (SELECT role_master.name FROM role_master WHERE role_master.id = employees.role LIMIT 1) AS role_name, `role`, (SELECT designation_master.name from designation_master WHERE designation_master.id = employees.designation) AS `designationname`, `designation`, (SELECT shift_management.name FROM shift_management WHERE shift_management.id = employees.shiftid LIMIT 1) AS working_shift, (SELECT company.name FROM company WHERE company.id = employees.companyid) as company_name,(CASE WHEN status = 0 THEN "Deactive" ELSE "Active" END) as _status, uuid FROM `employees` WHERE `companyid` = '+req.decoded.logedinuser.companyid
                }
                con.query(sql,function(err, result){
                    if(err)
                    {
    ...
    getAdvertisingPartnersList: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            connection.acquire(function(err, con){
                    ...
                    var sql = 'SELECT * FROM `advertising_partners` WHERE `companyid` = '+req.decoded.logedinuser.companyid
    ...
    getManagersList: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            connection.acquire(function(err, con){
                ...
                {
                    var sql = 'SELECT `id`, `name` FROM `employees` WHERE `role` IN (2, 5);';
                }
                con.query(sql,function(err, result){
                    if(err)
                    {
                    ...
                    if(officeLocations != undefined && officeLocations != null && officeLocations.length > 0)
                    {
                        ...
                        con.query('SELECT `id` FROM `attendance` WHERE `employee_id` = ? AND DATE_FORMAT(`date`, "%d-%m-%Y") = DATE_FORMAT(CURDATE(),"%d-%m-%Y") limit 1',[req.decoded.logedinuser.id],function(err, result){
                            ...
                            else
                            {
                                if(result != undefined && result.length > 0)
                                {
                                    con.query('UPDATE `attendance` SET `outtime`=?,`out_address`=?,`out_latlang`=?, `out_uuid`=? WHERE `id` = ?',[moment(req.body.time).format("HH:mm"), req.body.address, JSON.stringify({latitude:req.body.latitude, longitude:req.body.longitude}), req.decoded.logedinuser.uuid, result[0].id],function(err, result){
                                        if(err)
                                        {
                                            res.send({
                                                status:0,
                                    ...
                                    //moment(new Date()).format("HH:MM")
                                    con.query('INSERT INTO `attendance`(`employee_id`, `date`, `intime`, `in_address`, `in_lanLang`, `uuid`,`companyid`) VALUES (?,?,?,?,?,?,?)',[req.decoded.logedinuser.id,  moment(new Date()).format("YYYY-MM-DD"),moment(req.body.time).format("HH:mm"), req.body.address, JSON.stringify({latitude:req.body.latitude, longitude:req.body.longitude}), req.decoded.logedinuser.uuid, req.decoded.logedinuser.companyid],function(err, result){
                                        if(err)
                                        {
                                            console.log(err)
                                            res.send({
...
>>>>>>> e1eee262009a15657cc8a1b0335486333986ff80
    ...
    getAttendanceReportOfEmployees(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            connection.acquire(function(err, con){
        var sql = 'SELECT e.id, e.name, e.shiftid, (SELECT designation_master.name FROM designation_master WHERE designation_master.id = e.designation) AS _designation, (SELECT role_master.name FROM role_master WHERE role_master.id = e.role) AS _role, GROUP_CONCAT(a.id) AS att_ids FROM employees AS e INNER JOIN  attendance AS a ON a.employee_id = e.id WHERE e.status = 1 AND DATE_FORMAT(a.date,"%M-%Y") = "'+req.body.date+'" GROUP BY e.id';
            ...
            else
            {
                if(result && typeof result == 'object' && result.length > 0)
                {
                    for(var i = 0 ; i < result.length;i++)
                    {
                        ...
                        var attListSQL = 'SELECT id, DATE_FORMAT(`date`, "%d") AS att_date, DATE_FORMAT(`intime`, "%I:%i %p") AS in_time, DATE_FORMAT(`outtime`, "%I:%i %p") AS out_time, `uuid`, `out_uuid`, (CASE WHEN  DATE_FORMAT(attendance.intime,"%H:%i") > (SELECT DATE_FORMAT(ADDTIME((SELECT shift_management.intime FROM shift_management WHERE shift_management.id = '+result[i].shiftid+'), (SELECT shift_management.latemark_interval FROM shift_management WHERE shift_management.id = '+result[i].shiftid+')),"%H:%i")) AND DATE_FORMAT(attendance.intime,"%H:%i") < (SELECT DATE_FORMAT(ADDTIME((SELECT shift_management.intime FROM shift_management WHERE shift_management.id = '+result[i].shiftid+'), (SELECT shift_management.halfday_interval FROM shift_management WHERE shift_management.id = '+result[i].shiftid+')),"%H:%i")) AND attendance.outtime IS NOT NULL THEN 1 ELSE 0 END) AS latemark_status, (CASE WHEN attendance.employee_id ='+result[i].id+' AND DATE_FORMAT(attendance.date,"%M-%Y") = "'+req.body.date+'" AND (attendance.outtime IS NULL OR DATE_FORMAT(attendance.outtime,"%H:%i") < (SELECT DATE_FORMAT(shift_management.outtime,"%H:%i") FROM shift_management WHERE shift_management.id = '+result[i].shiftid+') OR (DATE_FORMAT(attendance.intime,"%H:%i") > (SELECT DATE_FORMAT(ADDTIME((SELECT shift_management.intime FROM shift_management WHERE shift_management.id = '+result[i].shiftid+'), (SELECT shift_management.halfday_interval FROM shift_management WHERE shift_management.id = '+result[i].shiftid+')),"%H:%i")))) THEN 1 ELSE 0 END) AS half_day_status FROM `attendance` WHERE `id` in ('+result[i].att_ids+')';


                        con.query(attListSQL,function(err, attendanceList){
                            if(err)
                            {
                                console.log(err)
                            }
                            else
                            {
                                ...
                                result[i]['attendanceList'] = attendanceList;
                            }
                    });
...
<<<<<<< HEAD
                    var sql = 'SELECT employees.id, employees.name, GROUP_CONCAT(DATE_FORMAT(attendance.date,"%D") ORDER BY attendance.date ASC SEPARATOR \', \') AS attendance_dates, GROUP_CONCAT(DATE_FORMAT(attendance.`intime`, "%H:%i") ORDER BY attendance.date ASC SEPARATOR \', \') AS attendance_intimes, GROUP_CONCAT(DATE_FORMAT(`attendance`.`outtime`, "%H:%i") ORDER BY attendance.date ASC SEPARATOR \', \') AS attendance_outtimes FROM attendance INNER JOIN employees ON attendance.employee_id = employees.id WHERE DATE_FORMAT(attendance.date,"%M-%Y") = "'+req.body.date+'" GROUP BY employees.id ORDER BY employees.id ASC';
=======
