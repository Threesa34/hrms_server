    ...
    getEmployeesAttendanceList: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            connection.acquire(function(err, con){
                    ...
                    var sql = 'SELECT DATE_FORMAT(`date`, "%d-%m-%Y") AS att_date, DATE_FORMAT(`intime`, "%h:%i %p") AS in_time, DATE_FORMAT(`outtime`, "%h:%i %p") AS out_time,`in_address`, `out_address`, (CASE WHEN  DATE_FORMAT(attendance.intime,"%H:%i") > (SELECT DATE_FORMAT(ADDTIME((SELECT shift_management.intime FROM shift_management WHERE shift_management.id = (SELECT (SELECT employees.shiftid FROM employees WHERE employees.id = attendance.employee_id LIMIT 1) FROM employees WHERE employees.id = attendance.employee_id LIMIT 1)), (SELECT shift_management.latemark_interval FROM shift_management WHERE shift_management.id = (SELECT (SELECT employees.shiftid FROM employees WHERE employees.id = attendance.employee_id LIMIT 1) FROM employees WHERE employees.id = attendance.employee_id LIMIT 1))),"%H:%i")) AND DATE_FORMAT(attendance.intime,"%H:%i") < (SELECT DATE_FORMAT(ADDTIME((SELECT shift_management.intime FROM shift_management WHERE shift_management.id = (SELECT (SELECT employees.shiftid FROM employees WHERE employees.id = attendance.employee_id LIMIT 1) FROM employees WHERE employees.id = attendance.employee_id LIMIT 1)), (SELECT shift_management.halfday_interval FROM shift_management WHERE shift_management.id = (SELECT (SELECT employees.shiftid FROM employees WHERE employees.id = attendance.employee_id LIMIT 1) FROM employees WHERE employees.id = attendance.employee_id LIMIT 1))),"%H:%i")) AND attendance.outtime IS NOT NULL THEN 1 ELSE 0 END) AS latemark_status, (CASE WHEN  ( attendance.outtime IS NULL OR DATE_FORMAT(attendance.intime,"%H:%i") >= (SELECT DATE_FORMAT(ADDTIME((SELECT shift_management.intime FROM shift_management WHERE shift_management.id = (SELECT employees.shiftid FROM employees WHERE employees.id = attendance.employee_id LIMIT 1)), (SELECT shift_management.halfday_interval FROM shift_management WHERE shift_management.id = (SELECT employees.shiftid FROM employees WHERE employees.id = attendance.employee_id LIMIT 1))),"%H:%i"))) THEN 1 ELSE 0 END) AS half_day_status FROM `attendance` WHERE `companyid` = '+req.decoded.logedinuser.companyid+' AND `employee_id` = '+req.body.employee_id+' AND DATE_FORMAT(attendance.date,"%M-%Y") = "'+req.body.date+'"';
