    ...
    getEmployeesAttendanceMonthlyReport: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            connection.acquire(function(err, con){
                    ...
                    var sql = 'SELECT employees.id, employees.name, GROUP_CONCAT(DATE_FORMAT(attendance.date,"%D") SEPARATOR \', \') AS attendance_dates, GROUP_CONCAT(DATE_FORMAT(attendance.`intime`, "%H:%i") SEPARATOR \', \') AS attendance_intimes, GROUP_CONCAT(DATE_FORMAT(`attendance`.`outtime`, "%H:%i") SEPARATOR \', \') AS attendance_outtimes FROM attendance INNER JOIN employees ON attendance.employee_id = employees.id WHERE DATE_FORMAT(attendance.date,"%M-%Y") = "'+req.body.date+'" GROUP BY employees.id';
