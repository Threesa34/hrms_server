...
module.exports = {
    ...
    getUserDetails: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            conn.query('SELECT *, (CASE WHEN ((employees.gender = "Male" OR employees.gender = "Other" or employees.gender = "" or employees.gender is null) AND employees.profilepic = "" OR profilepic is null) THEN "default.jpg" WHEN ((employees.gender = "Female") AND employees.profilepic = "" OR profilepic is null) THEN "default_f.jpg" ELSE employees.profilepic END) AS profile_pic  FROM `employees` WHERE `id` = '+req.params.userid, function(err, result){
                if(err)
                {
                    res.send({
                        status:0,
                ...
                con.query('SELECT `id` FROM `attendance` WHERE `employee_id` = ? AND DATE_FORMAT(`date`, "%d-%m-%Y") = DATE_FORMAT(CURDATE(),"%d-%m-%Y") limit 1',[req.decoded.logedinuser.id],function(err, result){
                    ...
                    else
                    {
                        if(result != undefined && result.length > 0)
                        {
                            ...
                            con.query('INSERT INTO `attendance`(`employee_id`, `date`, `intime`, `in_address`, `in_lanLang`,`companyid`) VALUES (?,?,?,?,?,?)',[req.decoded.logedinuser.id,  moment(new Date()).format("YYYY-MM-DD"),  moment(new Date(req.body.time)).format("HH:MM a"), req.body.address, JSON.stringify({latitude:req.body.latitude, longitude:req.body.longitude}), req.decoded.logedinuser.companyid],function(err, result){
                                if(err)
                                {
                                    res.send({
                                        status:0,
