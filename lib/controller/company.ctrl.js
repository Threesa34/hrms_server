...
module.exports = {
    ...
    getCompanyDetails: function(req, res)
    {
        if (req.decoded != undefined && req.decoded != null && req.decoded != '' && req.decoded.success == true) {
            ...
            {
                req.params.companyid =  req.decoded.logedinuser.companyid
            }
            
            connection.acquire(function(err, conn){
            conn.query('SELECT *, (SELECT name FROM `th_master`.`countries` WHERE `th_master`.`countries`.`id` = company.country) AS country_name, (SELECT name FROM `th_master`.`states` WHERE `th_master`.`states`.`id` = company.state) AS state_name, (SELECT name FROM `th_master`.`cities` WHERE `th_master`.`cities`.`id` = company.city) AS city_name, CONCAT("https://threesainfoway.net:8896/uploads/company/",logo) as logo1,logo FROM `company` WHERE `id` = '+req.params.companyid, function(err, result){
                if(err)
                {
                    res.send({
                        status:0,
                ...
                connection.acquire(function(err, con){
                    con.query('INSERT INTO `company` SET ?', companyDetails,function(err, result){
                        ...
                        else
                        {
                           ...
                           con.query('INSERT INTO `employees`(`name`, `mobile1`, `mobile2`, `email`, `address`, `password`, `role`, `companyid`, `createdby`, status) VALUES (?,?,?,?,?,?,?,?,?,?)', [companyDetails.owner, companyDetails.mobile1,companyDetails.mobile2,companyDetails.email1,companyDetails.address_line1+", "+companyDetails.address_line2, password,2,companyid, req.decoded.logedinuser.id, 1],function(user_err, user_result){
                                if(user_err)
                                {
