...
module.exports = {
    ...
    AuthenticateAgent: function(req, res)
    {
        connection.acquire(function(err, con){
            {
                var sql = 'SELECT * FROM `advertising_partners` WHERE `mobile1` = ? OR `mobile2` = ?';
            }
            con.query(sql,[req.body.mobile, req.body.mobile],function(err, result){
                if(err)
                {
                    console.log(err)
                    res.send({
        ...
        connection.acquire(function(err, con){
            ...
            con.query(sql,function(err, result){
                ...
                else
                {
                    ...
                    {
                        var json_file = './app/newsletters/html/'+req.params.id+'_html_template.html';
                        var filePath = 'newsletters/html/'+req.params.id+'_html_template.html';
                        fs.readFile(json_file, 'utf8', function(err, data){ 
                  
                            // Display the file content 
                            res.send({data:data, filepath: filePath, enquiry_form:result[0].enquiry_form}); 
                        });
                    }
                    else
                    {
                    ...
                    fs.readFile(json_file, 'utf8', function(err, data){ 
                        ...
                        // Display the file content 
                        res.send({data:data, filepath: filePath, enquiry_form:0}); 
                    });
                    }
